import ipaddress
import math
from lan import LAN
from router import Router
from switch import Switch
from prettytable import PrettyTable


# Calcul Prefix
def calculate_expression(n):
    return 32 - math.ceil(math.log2(n))

# Funcția pentru a crea LAN-uri
def create_lans(num):
    lans = {}
    for i in range(1, num + 1):
        lan_name = "LAN" + str(i)
        lan_instance = LAN(lan_name, i-1)  # Adăugăm indexul original
        lans[lan_name] = lan_instance
    return lans

# Sortarea LAN-urilor
def sort_lans_by_hosts(lans):
    return sorted(lans, key=lambda lan: lan.get_hosts(), reverse=True)

# Funcția pentru a înlocui numele unui LAN(Pt a adauga WAN-urile)
def replace_name(lans, actual_name, new_name):
    if actual_name in lans:
        lans[actual_name].set_name(new_name)
        lans[new_name] = lans.pop(actual_name)
    else:
        print(f"LAN-ul cu numele {actual_name} nu există.")
        
        

def main():
    ip_input = input("Introduceți clasa de IP: ")
    try:
        # Adresa IP cu prefix
        interface = ipaddress.ip_interface(ip_input)
        major_interface = interface
        
        # Obținerea adresei IP fara prefix
        ip_address = interface.ip
        
        # Crearea unui obiect IPv4Network fără prefix
        IPv4IP = interface.network
        
        networkAddress = IPv4IP.network_address
        broadcastAddress = IPv4IP.broadcast_address
        masca = IPv4IP.netmask
        
        # Crearea LAN-urilor și setarea numărului de gazde pentru fiecare LAN
        num_lans = int(input("NR LAN-uri: "))
        lans = create_lans(num_lans)
        for lan_name, lan in lans.items():
            num_lan_hosts = int(input(f"Hosts {lan_name}: "))
            lan.set_hosts(num_lan_hosts)
        
        # Sortarea LAN-urilor în funcție de numărul de gazde
        sorted_lans = sort_lans_by_hosts(list(lans.values()))
        
        # Parcurgerea LAN-urilor sortate și setarea adresei IP pentru fiecare LAN
        for i in range(len(sorted_lans)):
            current_lan = sorted_lans[i]
            if i == 0:
                n = current_lan.get_hosts()
                if n <= 2:
                    n += 2
                new_prefix = calculate_expression(n)
                new_network = ipaddress.IPv4Network(str(ip_address) + "/" + str(new_prefix), strict=False)
                current_lan.set_ip_address(new_network.network_address)  # Alege adresa LAN
                interface = ipaddress.ip_interface(f"{new_network.network_address}/{new_prefix}")
                current_lan.set_interface(interface)  # Interfața LAN cu tot cu prefix
                current_lan.set_network(new_network.network_address)  # Network
                current_lan.set_broadcast(new_network.broadcast_address)  # Broadcast
                current_lan.set_first_ip(new_network.network_address + 1)  # Primul IP
                current_lan.set_last_ip(new_network.broadcast_address - 1)  # Ultimul IP
                current_lan.set_mask(new_network.netmask)  # Masca
            else:
                previous_lan = sorted_lans[i - 1]
                previous_broadcast = ipaddress.IPv4Address(previous_lan.get_broadcast())
                new_start_ip = previous_broadcast + 1
                n = current_lan.get_hosts()
                if n <= 2:
                    n += 2
                new_prefix = calculate_expression(n)
                new_network = ipaddress.IPv4Network(str(new_start_ip) + "/" + str(new_prefix), strict=False)
                current_lan.set_ip_address(new_network.network_address)  # Alege adresa LAN
                interface = ipaddress.ip_interface(f"{new_network.network_address}/{new_prefix}")
                current_lan.set_interface(interface)  # Interfața LAN cu tot cu prefix
                current_lan.set_network(new_network.network_address)  # Network
                current_lan.set_broadcast(new_network.broadcast_address)  # Broadcast
                current_lan.set_first_ip(new_network.network_address + 1)  # Primul IP
                current_lan.set_last_ip(new_network.broadcast_address - 1)  # Ultimul IP
                current_lan.set_mask(new_network.netmask)  # Masca
        
        # Sortarea din nou a LAN-urilor pentru a păstra ordinea inițială
        sorted_lans.sort(key=lambda lan: lan.original_index)
        
        #adaugarea WAN-urilor##############################################################
        replace_name(lans, "LAN4", "WAN1")
        replace_name(lans, "LAN5", "WAN2")
        replace_name(lans, "LAN6", "WAN3")
        #adaugarea WAN-urilor##############################################################
        
        #Tabelul de subnetare
        subnet_table = PrettyTable()
        subnet_table.field_names = ["Denumire", "Adresa LAN", "Network", "Broadcast", "Primul IP", "Ultimul IP", "Masca"]
        
        for lan in sorted_lans:
            subnet_table.add_row([
                lan.get_name(),
                str(lan.get_interface()),
                lan.get_network(),
                lan.get_broadcast(),
                lan.get_first_ip(),
                lan.get_last_ip(),
                lan.get_mask()
            ])
        #print(subnet_table)
        
        # Crearea routerelor
        num_routers = int(input("Introduceți numărul de routere: "))
        routers = [Router(f"R{i+1}") for i in range(num_routers)]
        
        # Configurarea interfețelor fiecărui router
        used_ips = {}
        
        for router in routers:
            print(f"Configurați interfețele pentru {router.get_name()}:")
            for interface in router.interfaces:
                lan_name = input(f"{interface} conectat la: ")
                if lan_name.lower() == 'x':
                    router.set_interface(interface, None)
                elif lan_name in lans:
                    if lan_name not in used_ips:
                        used_ips[lan_name] = lans[lan_name].get_first_ip()
                    else:
                        used_ips[lan_name] += 1
                    router.set_interface(interface, used_ips[lan_name], lans[lan_name].get_mask())
                else:
                    print(f"{lan_name} nu este un LAN valid. Interfața {interface} nu va fi configurată.")
        
        # Afișarea detaliilor pentru fiecare router
        #print("\nDetalii routere:")
        routers_table = PrettyTable()
        routers_table.field_names = ["Denumire device", "Interfață", "Adresa IP", "Subnet Mask"]
        
        for router in routers:
            for interface in router.interfaces:
                ip_address, subnet_mask = router.get_interface(interface)
                if ip_address:  # Afișează doar interfețele configurate
                    routers_table.add_row([router.get_name(), interface, ip_address, subnet_mask])
        
       # print(routers_table)
        
        # Crearea switch-urilor
        num_switches = int(input("Introduceți numărul de switch-uri: "))
        switches = [Switch(f"SW{i+1}") for i in range(num_switches)]
        
        # Configurarea interfețelor fiecărui switch
        for switch in switches:
            print(f"Configurați interfața pentru {switch.get_name()}:")
            lan_name = input("Interfață conectată la: ")
            if lan_name in lans:
                ip_address = lans[lan_name].get_first_ip() + 1
                subnet_mask = lans[lan_name].get_mask()
                switch.set_interface(ip_address, subnet_mask)
                
                # Configurarea gateway-ului implicit pentru switch
                gateway_name = input("Gateway (ex: R1): ")
                router_interface = gateway_name.split(' ')[0]
                router_name = gateway_name.split(' ')[1] if len(gateway_name.split(' ')) > 1 else "Gig0/0"
                found_gateway = False
                
                for router in routers:
                    if router.get_name() == router_interface:
                        gateway_ip, _ = router.get_interface(router_name)
                        if gateway_ip:
                            switch.set_default_gateway(gateway_ip)
                            found_gateway = True
                            break
                
                if not found_gateway:
                    print(f"Gateway-ul {gateway_name} nu este valid. Gateway-ul implicit nu va fi configurat.")
            else:
                print(f"{lan_name} nu este un LAN valid. Interfața nu va fi configurată.")
        
        # Afișarea detaliilor pentru fiecare switch
        #print("\nDetalii switch-uri:")
        switch_table = PrettyTable()
        switch_table.field_names = ["Denumire device", "Interfață", "Adresa IP", "Subnet Mask", "Gateway implicit"]
        
        for switch in switches:
            switch_table.add_row([
                switch.get_name(),
                "Interfață",
                switch.get_interface(),
                switch.get_subnet_mask(),
                switch.get_default_gateway()
            ])
        
       # print(switch_table)
        
        print(f"Clasa de IP: {major_interface}  [N: {networkAddress} - {broadcastAddress} :B]")

        print("\tTabela de subnetare:")
        print(subnet_table)
        
        print("\n\tTabela de adresare:")
        print(routers_table)
        print(switch_table)
    
    except ValueError as e:
        print(f"Adresa IP introdusă nu este validă: {e}")

if __name__ == "__main__":
    main()
